...
```{r, echo = FALSE}
setwd("C:/Users/fbiney/Desktop/WaterProj/CourseReport_new")
```
...
lead and wastewater exposure in El Paso communities (census tract) using
geospatial heat maps and data from the climate and Economic Justice Tool
(CEJT) initiative [website](https://screeningtool.geoplatform.gov/en/about). The CEJT tool is a Biden-Harris
...
[(CEJT 2022)](https://www.federalregister.gov/documents/2021/02/01/2021-02177/tackling-the-climate-crisis-at-home-and-abroad#:~:text=Sec.%20223.%20Justice40,40-percent%20goal.). We also provided feedback on features that we thought were
great, features that we would like to see, and roadblocks we encountered
while implementing the Climate and Economic Justice Tool (CEJT)in it
beta phase from github.

# Exploration of CEJT for EL Paso county.

Figure 1 shows the CEJT result for a community in El Paso with zip code
...
![**FIGURE
1**](C:%5CUsers%5Cfbiney%5CDesktop%5CWaterProj%5CCourseReport_new%5CElpasoCejt2.PNG)

[CEJT
Tool](https://screeningtool.geoplatform.gov/en/#9.89/31.812/-106.4175)

# Summary of CEJT Tool
...
the Climate and Economic Justice Tool implementation process (CEJT).
...
On the implementation of the Climate and Economic Justice Tool (CEJT) in
...
-   Where in El Paso (census tract) has higher% of lead paint exposure.

-   Where in El Paso (census tract) has higher % waste water discharge
    exposure.

-   What is the effect of higher% of lead paint exposure on the health
    of the community.

-   What is the effect of higher% of waste water discharge exposure on
...
El Paso is known to have a high concentration of lead in the air and
...
percent diagnose Diabetes,percent diagnose Asthma, percent diagnose
...
data. Waste water discharged, life expectancy and media house hold
...
*Proximity to waste water discharged* were re-coded as binary factor
with categories *(yes,no)* for *(True,false)* respectively modeling
purpose.
...
not by the CEJT formula [site] ?for deciding who is disadvantage and
who is not?. Table [\*\*\*] below show that 5% of El Pasoans live in
lead painted house ,24% close to waste water discharge area and none
close to Hazard waste site. From Figure [###]...blue color. The
distribution of the water wast contain mostly high values.
...
p4<-EP_WQ_df %>% ggplot(aes(x=wastewater.dischg.gde90_LILE,fill= wastewater.dischg.gde90_LILE))+
  geom_bar()+labs(x = "Wastewater discharge >= 90%") +theme(legend.position="none")
...
eight categories
[ref](https://screeningtool.geoplatform.gov/en/methodology#:~:text=Under%20the%20current,the%20socioeconomic%20indicators).
...
```{r echo=FALSE}
# Waste water Dischr vs tract ----------------------------

g4<-ggplot(WQ_tract_tbl, aes(fill = wastewater.dischg.gde90_LILE  )) + 
  geom_sf()+
  labs(title='El Paso Communities with and without waste water Exposure',
       caption = "FIGURE 10")

g4+ scale_fill_discrete(name="Waste Water Exposure")
...
### Effect of high% waste water discharge on life Expectancey, Diabetie and Heart Disease
...
EP_WQ_Demo_df <- EP_Demo_df %>% 
  mutate(Lead = EP_WQ_df$Ld.Paint.gde90_LILE,
         'Waste water' = EP_WQ_df$wastewater.dischg.gde90_LILE)



pp1 <- EP_WQ_Demo_df %>% 
  ggplot(aes(x=`Waste water`,y=life.expectcy_yrs ,fill=`Waste water`))+
  geom_boxplot() + labs(y = "Life Expect. Yrs.") +theme(legend.position="none")


pp2<- EP_WQ_Demo_df %>% 
  ggplot(aes(x=`Waste water`,y=Diabetes.Adults.age18.diag ,fill=`Waste water`))+
  geom_boxplot()  + labs(y = "Adult diagnosed diabetes") +theme(legend.position="none")
...
pp3<- EP_WQ_Demo_df %>% 
  ggplot(aes(x=`Waste water`,y= Heart.dis.Adults.age18    ,fill=`Waste water`))+
  geom_boxplot() + labs(y = "Adullt diagnosed heart disease.") +theme(legend.position="none")
...
```{r echo = FALSE}
annotate_figure(plt2, top = text_grob("Waste water's effect Medical on Condition", 
               color = "black", face = "bold", size = 14),
...
# Conclusion 
This work explores the distribution, correlation, and effect of exposure to lead and wastewater on heart disease, diabetes, and life expectancy in El Paso communities. We accomplished this by using geospatial heat maps, boxplot, data from the CEJT initiative, and the Tigris package for CRAN. 
